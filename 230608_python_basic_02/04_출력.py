# 출력
# 값을 여러 개 출력하기
i=0
'''
print에는 변수나 값 여러 개를 ,(콤마)로 구분하여 넣을 수 있음
print(값1, 값2, 값3)
print(변수1, 변수2, 변수3)
'''
print(1,2,3)
print('뱀','고양이')
a=10
print(10,a,'10')
# 여러 개의 값을 넣을 때, 기본적으로 print 시에 ' '(스페이스)로 구분해준다.
# 값을 여러 개 출력할 때, print 함수를 여러 번 쓰지 않아도 됨.

# 구분자를 변환
'''
값 사이에 공백이 아닌 다른 문자를 넣고 싶을  때는
print의 sep에 문자 또는 문자열을 지정(sep는 구분자라는 뜻의 **sep**arator에서 따옴)
* print(값1, 값2, sep='문자 또는 문자열')
* print(변수1, 변수2, sep='문자 또는 문자열')
'''
print(1,2,3, sep=',') # ,(콤마)로 연결
print(1,2,3) # 공백으로 연결
print('그냥','공백없이','붙어있었으면',sep='')
print('그냥','공백없이','붙어있었으면',sep=' ') # 이게 기본 sep
print('이건','콤마로','구분한 것',sep=',')

# 줄바꿈 활용하기
# print에 값을 여러 개 지정 하면 한 줄에 모든 값이 출력
print("개","고양이","돼지")
# sep => 개행 문자(줄바꿈 문자 = Enter = \n) 라는 특별한 문자를 지정 하면...
print(1,2,3,sep='\n') # 값을 한 줄에 하나씩 출력
print("5\n6\n7") # 비슷한 결과 \n은 문자열에 섞이면 줄바꿈 역할읋 함.

# end 사용하기
# print("...", sep=" ", end="\n")
'''

'''
print(1)
print(2)
print(3)
print("위 123과 아래 123의 차이를 확인")
print(1,end='') # \n->''
print(2,end='') # \n->''
print(3) # \n->''

# end를 스페이스로 구분한 것
print(1,end=' ') # \n->' '
print(2,end=' ')
print(3)


# ---------------
# 힌국은 숫자를 만의 단위로 끊어서 표현
# 미국은 숫자를 천의 단위로 끊어서 표현 -> 파이썬 =>미국
print(1000000)
print(1_000_000) # 천의 단위씩 _로 나눠서 표현할 수 있다
# 3자리마다 _(밑줄 문자)를 사용해서 큰 숫자 나눠서 표현 가능