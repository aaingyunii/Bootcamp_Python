# 변수와 입력
## 변수 만들기
x = 10 # x라는 변수에 10이라는 값을 할당/대입
'''
* x = 10 이라고 입력하면, 10이 들어있는 변수 x가 만들어짐
즉, 변수이름 = 값 형식 -> 변수가 생성(선언)되는 동시에 값을 할당(저장)
* 변수 이름을 지을 때, 일종의 규칙이 있음. (=제약조건)
: 영문과 숫자를 사용할 수 있음.
: *****대소문자를 구분한다. (Apple != apple) 다른 표현!
: 문자로 시작해야함. (숫자 시작X)
: _(언더바)로 시작 가능.
: 특수문자(+,-,*,/,$,%,@ 등)는 사용 불가
: 파이썬의 키워드(미리 사용을 정의한 단어들)는 사용할 수 없음. ex) if, for, while, and, or,...
'''
print(x) # 10
# 변수에는 숫자뿐만 아니라 문자,문자열도 대입 가능.
y = "양평해장국"
print(y) # 양평해장국
print(x,y) # 전체 스크립트가 실행되는 한 변수의 값은 유지된다.

# 실수, 정수,... <-자료형 : 데이터가 가지고 있는 특성의 분류.
# 변수는 자신이 가지고 있는 값에 따라 자료형이 달라짐.

#자료형 확인하기
print(type(x),type(y))
print(x)
x = "순대국밥"
print(x) # 순대국밥 -> 변수에 다른 값을 새롭게 대입 가능.

# int(=integer) 정수, str(=string) 문자열
# 변수는 값에 따라 자료형이 결정된다.

# 할당연산자 (대입연산자) '='
'''
* 수학에서 '=' : 양변의 값이 같다라는 뜻
* 프로그래밍 언어에서는 '='은 변수에 값을 할당(assignment)한다라는 의미,
등호의 역할은 '=='(동등연산자)가 한다.
* R은 '=' 동등연산자, <- 대입연산자
'''
...
print(x, y)
x, y = y, x  # 튜플 형식, 파이썬은 이런 식으로, 두 변수의 위치와 값을 변환.
print(x, y)
