...
'''
시퀀스 자료형 -> 슬라이싱 -> 무엇인가의 일부를 잘라낸다.
-> 시퀀스 슬라이스 -> 시퀀스 객체의 일부를 잘라내는 것. (원본에 영향 X)
* 시퀀스객체[시작인덱스:끝인덱스] (시작인덱스는 포함하지만 끝인덱스는 포함 X
'''
print(a)
print(a[3:8]) # 인덱스 3부터 7까지 슬라이싱. 시작 인덱스 포함 O, 끝 인덱스는 포함 X => range와 비슷..
print(a) # a 리스트 원본에 영향 X
# 끝 인덱스는 가져오려는 범위에 포함 X -> 실제로 가져오려는 인덱스+1 해야함.
