...
for i in range(101,201): # -> 101~ 200 까지 i 변화
    if i == 151:
        print()
    print(i,end=" ")
print("\n")
...
for i in range(20, 10, -1):
    print(i,end=" ")
print()
"""range(10, 0)과 같이 시작하는 숫자를 큰 숫자로 지정하고 끝나는 숫자를 작은 숫자로 지정하면 숫자가 감소할 것 같은데, 
실행을 해보면 아무것도 출력되지 않습니다. 왜냐하면 range는 숫자가 증가하는 기본 값이 양수 1이기 때문입니다."""
...
그리고 다시 조건식을 판별하여 참(True)이면 코드를 계속 반복하고, 거짓(False)이면 반복문을 끝낸 뒤 다음 코드를 실행합니다.
...
w= 0 # 반복이 진행될 떄마다 변화할 값 -> 시작점
while w < 100:  # while 조건식 == True 일 때 반복 -> 끝점
# while not w >= 100: # 위의 식이랑 동일
    print("'Hello, world!'")
    w +=1 # 위의 식만 있으면 무한루프 -> 변화식을 통해 반복 수를 조절 -> 증가폭

"""먼저 while 반복문에 사용할 변수 i에 0을 할당합니다. 그리고 while에는 조건식만 지정하면 됩니다.
...
### 초깃값을 감소시키기
w= 100 # 반복이 진행될 떄마다 변화할 값 -> 시작점
while w > 0:  # while 조건식 == True 일 때 반복 -> 끝점
# while not w <= 0: # 위의 식이랑 동일
    print(w,"'Hello, world!'")
    w -=1 # 위의 식만 있으면 무한루프 -> 변화식을 통해 반복 수를 조절 -> 증가폭


"""100부터 1까지 반복해야 하므로 먼저 i에 100을 할당합니다. 그다음에 while의 조건식은 i > 0과 같이 지정하여 1까지만 반복하도록 만듭니다. 
또한, 변화식을 i -= 1로 지정하여 변수의 값을 감소시킵니다. 
이렇게 하면 i가 100부터 1까지 감소하면서 반복합니다. 만약 i가 0이되면 i > 0은 거짓(False)이므로 반복문을 끝냅니다.
...
"""input으로 입력 값을 받아서 count 변수에 저장했습니다. 
그리고 조건식에서 count > 0와 같이 지정하여 count에 들어있는 값만큼 반복하도록 만들었습니다. 
여기서는 변수 i 대신 count를 바로 사용하므로 변화식을 count -= 1로 지정하여 반복할 때마다 count를 감소시키고 count가 0이 되면 반복문을 끝냅니다.

## 반복 횟수가 정해지지 않은 경우

지금까지 조건식에서 반복 횟수를 정한 뒤 변수 i를 증가시키거나 감소시켜서 while 반복문을 사용했습니다. 
...
import random # 파이썬 내장 라이브러리 -> 이미 설치되어있는 것.
# print(random.random())  # 0~1 사이의 임의의 실수 출력. [0,1), 0 포함 O, 1 포함 X
...
"""
print(random.randint(1,6))  # 1~6, 1,6 둘다 포함
print()
...
while dice != 3:
    ...
    print("주사위에서",dice,"이(가) 나왔습니다.")
    c +=1
    if dice == 3:
        print("당신은",c,"번째만에 3을 뽑았습니다.")


"""while 반복문에서 i != 3과 같이 조건식을 지정하여 3이 아닐 때 계속 반복하도록 만듭니다. 
그리고 반복할 코드에서 i = random.randint(1, 6)과 같이 1과 6 사이의 난수를 생성하여 변수 i에 저장합니다.

이제 이 코드를 여러 번 실행해봅니다. 실행 결과를 보면 정수가 무작위로 생성되어 실행할 때마다 반복 횟수가 달라지는 것을 볼 수 있습니다.
...
print(random.choices([1,2,3,4,5],k=2)) # 중복 상관없이 한번에 k개수 뽑는 거
# numpy -> 비복원 추출, 중복 삭제해서 추출.
...
for i in range(100):
    ...
    print("당신의 잔액은",money,"입니다.")
    if i == 99:
        print("가능한 모든 횟수를 사용하셨습니다.")
        print("당신은 행운아군요! :",money,"원을 가지고 계십니다.")
    if money <=0:
        print("당신은",i,"번 도전하셨습니다.")
        print("돈이 앵꼬 났어요! :",money,"원")
        break

"""먼저 while에 True를 지정하여 무한 루프를 만들고, 그 안에서 i를 1씩 증가시킵니다. 
그리고 if를 사용하여 i가 100이 될 때 break를 실행합니다. 이렇게 하면 무한 루프라도 0부터 99까지만 화면에 출력합니다. 
