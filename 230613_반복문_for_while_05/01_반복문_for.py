# 반복문
a=""
## for 와 range
'''
반복문 : 특정한 규칙에 맞춰서 특정한 코드를 반복적으로 실행.
* for ... in : 반복문을 위한 문법.
for 변수 in range(횟수): # range 안에(in) 있는 요소들을 '변수명'으로 하나씩 꺼내와서
                        # 들여쓰기 안에 사용하겠다.
    반복할 코드 (반복문)
'''
for i in range(10):
    # i-> 0 ~ 9
    print("자니?",i) # range에서 요소들을 하나씩 꺼내서 i에 넣고 사용.

'''
반복문의 변수 i : 루프 인덱스 -> loop index -> i.

# for in + range 반복
# range의 특성? : range(끝점), range(시작점, 끝점), range(시작점, 끝점, 증가폭)

for 변수명(i) in range(시작점, 끝점):
    반복할 코드
'''

# 5부터 10까지 합해주는 코드 (1)
a = 0  # 모든 합을 기록할 변수 (반복문 밖에 둠)
for i in range(5, 11):
    print(i) # 5 ~ 10 <- 순서대로 대입

# 5부터 10까지 합해주는 코드 (2)
a = 0  # 모든 합을 기록할 변수 (반복문 밖에 둠)
for i in range(5, 11):
    # print(i) # 5 ~ 10 <- 순서대로 대입
    # i는 순회할 때마다 새로운 값으로 변화
    # a = a + i
    a += i
    print(i, a)
print(a)

# 5부터 10까지 합해주는 코드 (3)
a = 0  # 모든 합을 기록할 변수 (반복문 밖에 둠)
for i in range(5, 11):
    # print(i) # 5 ~ 10 <- 순서대로 대입
    # i는 순회할 때마다 새로운 값으로 변화
    # a = a + i
    # i가 홀수 일떄만 더한다면?
    if i % 2 == 1:
        # if i % 2 : -> 이것도 성립한다. why? i%2 에서 1 나오는 것들은 홀수이며 이것은 True 값이기 때문에 성립한다.
                    # 반대로 짝수라면, i % 2 == 0, Fasle 값이 반환된다.
        a += i
        print(i, a)
print(a)

# 증가폭
# for 변수명 in range(시작, 끝, 증가폭):
# for i in range(0, 10, 2):
# for i in range(1, 10, 2):
for i in range(1, 10, 3):
    print(i)  # 0, 2, 4, 6, 8

# 뒤집기
# for i in range(1, 10, -1): # range(1, 10, -1) -> 빈 range. -> 음수 증가폭 -> 시작점 > 종료점
for i in range(9, 0, -1):  # (10, 1, -1) -> 시작점은 포함, 끝점은 미포함.
    print(i)

# reversed -> 시퀀스 형태의 자료형을 뒤집어 주는 기능
'''
- for 변수 in reversed (range(횟수))
- for 변수 in reversed (range(시작, 끝))
- for 변수 in reversed (range(시작, 끝, 증가폭))
'''
print("for i in range(9,-1,-1) :")
for i in range(9,-1,-1):
    print(i,end=" ")
print("\n")

print("for i in reversed(range(10)) :")
for i in reversed(range(10)):
    print(i,end=" ")
print("\n")

print("for i in range(10)[::-1]:")
for i in range(10)[::-1]:
    print(i, end=" ")
print("\n")

print("for i in range(10): print(9 - i, end=\" \")")
for i in range(10):
    print(9 - i, end=" ")
print("\n")

# 시퀀스 객체로 반복하기 (리스트, 튜플, 문자열...)
a = ["아무1", "아무2", "아무3"]
for v in a: # value -> v.
    print(v)

fruits = ["사과", "배", "포도"]
# for fruit in fruits: # 리스트, 튜플 -> 시퀀스 (복수형) => for를 통해서 반복 호출하는 변수명 -> 단수.
for f in fruits: # 리스트, 튜플 -> 시퀀스 (복수형) => for를 통해서 반복 호출하는 변수명 -> 단수.
    print(f)

# 문자열의 경우
text = "작은 것들을 위한 시"
# for c in text: # c : character
# for c in reversed(text): # c : character
for c in text[::-1]: # c : character
    print(c)

# 인덱스를 사용한 조회
for i in range(len(text)):
    # range(len(text)) -> text의 정수 인덱스 정록
    # -> text[i] : 반복문으로 조회하면 -> 요소 하나하나를 검색
    # print(i,end="")
    print(text[i], end=" ") # text가 가지고 있는 인덱스의 목록
print()

for i in range(len(text)): # 시퀀스의 길이를 바탕으로 인덱스 목록을 만들어서
    print(text[i],end=" ")# 해당 인덱스로 호출할 수 있는 값들을 출력한 코드
print()

for i in text: # 시퀀스 안에 있는 요소들을 하나씩 호출해서
    print(i,end=" ") # 출력한 코드
print()


# enumerate -> 인덱스, 값 -> 튜플 쌍을 만들어 주는 함수
for i in enumerate(fruits):
    print(i) # (인덱스, 요소의 값)

for i in enumerate(fruits):
    a,b = i # i는 튜플 -> 왼쪽에 자리수를 맞춰서 변수를 둬서 해체 => 튜플 언패킹.
    print("a == 인덱스 :",a,"/ b == 요소의 값 :",b) # a index / b elemnet

print("for i,j in enumerate(fruits) :")
for i,j in enumerate(fruits):
    print("Index :",i,"\nElement :",j)
    # for문 시작부터 언팩킹으로 할당이 가능!