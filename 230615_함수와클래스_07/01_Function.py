...
my_fun()  # 소괄호!
# my_fun -> () -> 응답
# 함수 호출 : 함수를 사용한다.(  ()  ) -> 함수 호출, call


# 함수를 만들고 호출 -> 함수를 만들기 전에 함수 호출 X

# my_fun2()   # NameError: name 'my_fun2' is not defined. Did you mean: 'my_fun'?
...
```
"""
# def solution(매개변수) : ~-> 프로그래머스 답안지 형태

def greeting(name, time):
    print(f"안녕하세요 저는 {name} 입니다. 지금 {time}시 네요")

greeting("안코딩",12)


# 인수(인자) 와 파라미터 -> 값 자체가 인수, 그 값이 들어가서 전달되는 '변수' -> 매개변수 파라미터
...
x = add(10, 100)    # 안으로 들어오는 값은 '인자'라고 불리고, 내부에서는 '매개변수'를 통해 호출
...
def my_fun(a,b): # concat - 문자열 연결
    # 숫자가 들어오면 안되는 함수
    # 숫자가 들어오면 경고문 print 후 빈 문자열 반환

    if type(a) != str or type(b) != str:
        print("입력하신 것중 문자열이 아닌 것이 있습니다.")
        return "Error"   # 이 코드가 실행되면 함수가 멈춘다.
    print("제대로 처리중 입니다.")
    return a+b  # 문자열
...
"""
import random
random.randint(1,6)    # 1은 시작하는 수 , 6은 끝나는 수(포함)
l = [1,100]
print(random.randint(*l))   # * : 별표, 애스터리스크,..-> 언패킹
...
print(range(1,5))
print(*range(1,5))  # range(1,5)를 언패킹해서 하나씩 출력
print("가변 인수 함수 만들기")
print(*"가변 인수 함수 만들기",)  # 한 글자씩 출력
...
```
"""
def order(*menu) : # 이렇게 전달받은 인자들을 오히려 패킹시켜서 처리가 가능하다.
    for i in menu:
        print(f"메뉴는 {i}입니다.")

order("돈까스","우동","새우튀김")
# "돈까스","우동","새우튀김" <- 인자로 전달된 것들을 패킹시켜서 menu의 변수에 집어넣음.
def my_sum(*nums):
    answer = 0
    for i in nums:  # nums : 튜플 -> 시퀀스
        answer +=i
    return answer
print(my_sum(1,2,6453,2323,5,2,6768778,21,2000313,2,4,88))
...
l = list(range(10))
print(l)    # 리스트로 출력
print(*l)   # 한 묶음이지만, 리스트의 요소들을 하나씩 찢어서 출력
...
def order(category, *args): # 무조건 가변 인수를 나타내는 매개변수보다(*표시 있는 변수), 고정 매개변수가 앞에 존재
    print(f"{category}의 종류의 음식을 원하는 군요")
    print(*args,"를 드리겠습니다.")
...
def order(category, price):
    print(f"{category} 종류의 음식을 {price}원 주문하셨습니다.")

order("중식",15000)

# def order(*category, menu):  # 무조건 가변 인수를 나타내는 매개변수보다(*표시 있는 변수), 고정 매개변수가 앞에 존재
#     print(f"{category}의 종류의 음식을 원하는 군요")
#     print(*args, "를 드리겠습니다.")
#
#
# order("일식", "우동", "돈까스", "모밀")
#   TypeError: order() missing 1 required keyword-only argument: 'menu'
...
print(*d)   # 딕셔너리형태에서 * 하나는  key를 사용한다는 뜻
...
order(first = "짜장면",second = "우동",third="만두")
print()
...
# (아무것도X)  (*붙은거)       (**붙은거)
def func(param1,param2,param3, *args, **kwargs):
    pass
...
함수에서 위치 인수를 받는 `*args`와 키워드 인수를 받는 `**kwargs`를 함께 사용할 수도 있습니다. 
대표적인 함수가 print인데 print는 출력할 값을 위치 인수로 넣고 sep, end 등을 키워드 인수로 넣습니다. 
다음과 같이 함수의 매개변수를 `*args`, `**kwargs`로 지정하면 위치 인수와 키워드 인수를 함께 사용합니다.
"""
...
def greeting(word = "안녕 좋은아침!"):
    print(word)

greeting()
greeting("좋은 저녁이야")
greeting(word="좋은 날씨야")
print()
...
# def 함수명(기본값이 없는 매개변수, *위치가변인자매개변수, 기본값이 있는 매개변수=(기본값), **키워드가변인자매개변수):
```
...
# map -> 입력 -> input().split() => int => 괄호를 넣은 int가 아니라 그냥 int를 집어넣었다. => 아직 실행하지 않은 준비 상태의 함수
