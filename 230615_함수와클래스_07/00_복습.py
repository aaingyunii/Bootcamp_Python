...
l = list(range(0,10,2))
print(l)
l.append(100)   # 맨 끝에 요소 추가
print(l)
l.extend([200,10000])   # 새롭게 추가 하려는 리스트를 '한개씩' 반복적으로 끝에 추가
print(l)
l.insert(0,500)  # 인덱스 와 값
l.insert(4,500)  # 인덱스 와 값
l.insert(3,600)
print(l)
...
print("Remove")
print(f"l : {l}")
l.remove(600)  # 별도로 인덱스를 주지 않아도...
print(f"l : {l}")
# 같은 값이 2개 이상 있을 때?
l.remove(500)  # 왼쪽에서부터 먼저 발견된 것부터 삭제
print(f"l : {l}")
...
"""
print(f"r :{r}")    # r : random.randchoices(range(5),k=5)
print(f"min(r) :{min(r)}")
print(f"max(r) :{max(r)}")
print(f"sum(r) :{sum(r)}")
print(f"T calculate AVG r -> sum(r) / len(r) : {sum(r) / len(r)}")
...
print("리스트 컴프리헨션에 조건문을 추가한 것.")
x = [i**2 for i in range(10) if i%2 == 0]    # 짝수인 애들
print(f"x : {x}")
...
print(f"s2.find('나') : {s2.find('나')}") # 찾는 문자열이 없으므로 -1 반환
print(f"s2.find('승강장') : {s2.find('승강장')}") # 찾는 문자열의 가장 왼쪽의 인덱스 값을 반환
print(f"s2.find('승') : {s2.find('승')}") # 찾는 문자열의 가장 왼쪽의 인덱스 값을 반환
...
print(f"d : {d}")
print(d.get("name"))    # 없으면 None으로 기본 반환
...
for index, (k, v) in enumerate(d.items()):
    print(f"index: {index}, key : {k}, value : {v}")
