...
# ten_div("문자열")  # TypeError: unsupported operand type(s) for /: 'int' and 'str'
ten_div2("문자열") # 어떤 에러든 상관없이 "에러가 났습니다." 출력 후 0 값을 반환.
print()
...
지금까지 숫자를 0으로 나눴을 때 에러, 리스트의 범위를 벗어난 인덱스에 접근했을 때 에러 등 파이썬에서 정해진 예외만 처리했습니다. 
이번에는 우리가 직접 예외를 발생시켜 보겠습니다.

예외를 발생시킬 때는 raise에 예외를 지정하고 에러 메시지를 넣습니다(에러 메시지는 생략할 수 있음).

* `raise 예외('에러메시지')`
"""
v = int(input("숫자를 입력하세요 : "))
# 1000 이상 입력해야 작동
# 1000 미만 입력 시 에러 발생을 원함

print(f"{v}(을)를 입력하셨습니다.")
# if v < 1000:
#     raise Exception("1000 이상 입력해야합니다.")
# else:
#     print("Good Game \n")
#
try:
...
"""안에 try except가 없는 상태에서 raise로 예외를 발생시켰습니다. 
...
try:
    try:
        ...
        print("1000원이 넘게 넣어주세요")    # 여기도 에러 처리
        raise e  # 여러 번 처리해야되는 에러
except:
    print("에러가 발생 했었네요?")   # 밖에서도 에러 처리.

print()

"""함수 안에서 발생한 예외를 함수 안의 except에서 한 번 처리하고, raise로 예외를 다시 발생시켜서 상위 코드 블록으로 넘겼습니다. 
