...
물론 집, 자동차, 나무 등도 클래스로 표현할 수 있습니다. 특히 프로그래밍에서는 현실 세계에 있는 개념들뿐만 아니라 컴퓨터 안에서만 쓰이는 개념들도 클래스로 만들어서 표현합니다.
웹 브라우저에서 내용이 길어지면 보이는 스크롤 바, 프로그램에서 주로 볼 수 있는 버튼, 체크 박스 등이 대표적입니다.

지금까지 나온 기사, 마법사, 궁수, 사제, 집, 자동차, 나무, 스크롤 바, 버튼, 체크 박스처럼 특정한 개념이나 모양으로 존재하는 것을 객체(object)라고 부릅니다.
그리고 프로그래밍으로 객체를 만들 때 사용하는 것이 클래스입니다.

그럼 게임의 기사 캐릭터를 클래스로 표현하려면 무엇이 필요할까요?
간단합니다. 일단 게임 캐릭터는 체력, 마나, 물리 공격력, 주문력 등이 필요합니다. 그리고 기사 캐릭터는 칼로 베기, 찌르기 등의 스킬이 있어야 합니다.
...
따라서 현실 세계의 복잡한 문제를 처리하는데 유용하며 기능을 개선하고 발전시킬 때도 해당 클래스만 수정하면 되므로 유지 보수에도 효율적입니다.
...
"""
class MyClass:
    def hello(self0):   # 클래스 안의 함수 -> 메서드라 지칭
        print("Hello world")

mc = MyClass()  # 함수 마냥 '실행'
mc.hello()  # 해당 클래스에 소속된 함수-기능

"""그럼 이 클래스를 사용해봐야겠죠? 다음과 같이 클래스에 ()(괄호)를 붙인 뒤 변수에 할당합니다.

* `인스턴스 = 클래스()`
# 클래스 'B'를 통해서 객체를 만들면, (A) A라는 객체는, 클래스 'B'에 의해서 생성된 인스턴스다.
...
클래스는 객체를 표현하는 문법이라고 했는데, 클래스로 인스턴스를 만든다고 하니 좀 헷갈리죠? 
사실 인스턴스와 객체는 같은 것을 뜻합니다. 보통 객체만 지칭할 때는 그냥 객체(object)라고 부릅니다. 
하지만 클래스와 연관지어서 말할 때는 인스턴스(instance)라고 부릅니다. 그래서 다음과 같이 리스트 변수 a, b가 있으면 a, b는 객체입니다. 
...
지금까지 클래스에서 메서드를 만들고 호출해보았습니다. 이번에는 클래스에서 속성을 만들고 사용해보겠습니다. 
...
class Student:
    def __init__(self):     ## __init__ -> 해당 클래스의 인스턴스가 생성되면, 자동으로 실행
        # class -> 클래스명() -> 생성자
        print("Student가 생성되었습니다.")
        # self -> 클래스로 생성된 자기 자신
        # self.변수명 -> 클래스로 생성된 객체(인스턴스)에 소속된 변수 == 속성 대입이 가능.
        self.name = "아메리카노"

s = Student()   # Student 클래스로, 인스턴스를 생성해서 객체를 s라는 변수에 할당
print("인스턴스 생성되서 자동으로 발생된 self 변수 :",s.name)
...
여기서 `__init__`의 매개변수 self에 들어가는 값은 Person()이라 할 수 있습니다. 그리고 self가 완성된 뒤 james에 할당됩니다. 
이후 메서드를 호출하면 현재 인스턴스가 자동으로 매개변수 self에 들어옵니다. 그래서 greeting 메서드에서 print(self.hello)처럼 속성을 출력할 수 있었던 것입니다.
...
class Store:
    def __init__(self,location):    # self <- 건들지 않는 선에서...
        # 외부에서 대입하고 싶은 매개변수 -> location
        self.title = "중화반점"
        self.menu = ["소꼬리무침","가지무침","계란프라이스테이크"]
        # self.location = "신대방삼거리점"
        self.location = location

store3 = Store(location="신삼")
print("Store3 :",store3.title,store3.menu,store3.location)

store4 = Store(location="대방")
print("Store4 :",store4.title,store4.menu,store4.location,"\n")
...
store1 = BungeoppangStore('민트초코붕어빵 1호점',    # name
                          ...
                          james=5000, john=2000, kate=500   # 키워드가변인자 => kwargs => 방문자 리스트
                          )
...
class Store:
    ...
    def __init__(self,name="붕어빵 스토어"):
        # self에 아무것도 대입 되지 않았다면...자체 Class에 소속된 클래스 변수들이 먼저 할당

        # self.~~ -> 필요한 변수들을 넣어서 사용
        # self.속성 => 인스턴스 속성 => 변수 -> 인스턴스 (객체) => 게별적으로 존재하는...
...
class Car:
    ...
    def __init__(self,owner):
        self.owner = owner
        print(f"이 차의 브랜드는 {Car.brand}이고, 차주는 {self.owner} 입니다.")
car1 = Car("김코딩")
car2 = Car("박코딩")
car3 = Car("안코딩")
print("\n")
...
class ElectricCar(Car): # (Car) -> 이미 만들어놓은 Car클래스를 가지고와서 재사용!
    def __init__(self):
        self.fuel = "전기"
    def charge(self):   # 이것은 Car 클래스에 영향없이 독립적으로 ElectricCar 클래스 내에서만 사용 가능한 메서드!
        print("충전 합니다")

c2 = ElectricCar()
c2.drive()  # ElectricCar 클래스에는 따로 drive 함수를 정의하지 않았지만,
            # Car 클래스를 '상속'받았기 때문에 따로 정의하지않아도, Car 클래스의 메서드 'drive'를 사용할 수 있다.
c2.charge()
...
class PythonProgrammer(Programmer):
    # Programmer.__init__ -> 새롭게 정의한 __init__ 덮어씌워진다. => 오버라이딩
...
print("-------------------------------------")
# print(lee.habit,lee.money)  # TypeError: PythonProgrammer.__init__() missing 1 required positional argument: 'habit'
# print(lee.habit,lee.money)  # AttributeError: 'PythonProgrammer' object has no attribute 'money'
...
class PythonProgrammer(Programmer):
    ...
    def __init__(self,habit):
        ...
        # Programmer 클래스
        ## Good
        super().__init__()  # self와 연결이 되어서
        print(self.money,self.habit)
        # 오버라이딩 - > 같은 이름의 메서드를 재정의
        self.habit = habit

lee = PythonProgrammer("손톱 물어뜯기")
# Programmer __init__ :  -> PythonProgrammer의 __init__ 에서 super를 통해 Programmer 클래스의 __init__가 실행되었다.
...
"""
print("Human MetaHuman MetaMetaHuman")
class Human:
    def run(self):
        print("달려간다")
...
class MetaMetaHuman(MetaHuman):
    def run(self):
        ...
        print("나는 MetaMetaHuman, 상당히 더 빠르게 달려간다")
...
-> 같은 이름의 새로운 함수(메서드)를 지정해서 다른 용도로 메서드를 사용하는 것
...
class LuxuryFligth(Flight):
    pass    # 상속 받은 그대로 사용.

class ZetFlight(Flight):
    def fly(self):
        print("제트엔진으로 날아간다.")   # 개조를 시킴 -> 오버라이딩

f1 = Flight()
f2 = ZetFlight()
f3 = LuxuryFligth()
fligth_list = [f1,f2,f3]    # 모든 요소가 Flight를 상속 받은 클래스들.

for i in fligth_list:
    i.fly()     # Flight를 상속 받았기 떄문에
                # Flight에서 정의된 fly() 함수가 공통적으로 가지고 있다고 생각.
    ### 여기서 ZetFlight 의 경우,
    ### fly 함수를 재정의, 본연의 클래스 특징에 맞게 수정했다.
    ### -> 이를 오버라이딩이라고 지칭하는 것이다.
    print()